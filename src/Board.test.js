...
describe('Board with Backend', () => {
  test('fetches game state', async () => {
    axios.get.mockResolvedValue({
      data: {
        id: 1,
        name: 'Monopoly Game',
        players: [{ id: 1, name: 'Player 1', properties: [], piece: 'Thimble', balance: 15000000, position: 0, side: 'bottom' }],
        properties: [{ id: 1, name: 'St. Charles Place', pos: 11, group: 'purple', color: 'purple', houses: 0 }],
      },
    });
    render(<Board />);
    expect(await screen.findByTestId('property-11')).toBeInTheDocument();
  });
});